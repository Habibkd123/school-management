    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {homework.map((hw) => {
          ...
          return (
            <div
              key={hw.id}
              className="bg-white border border-border rounded-xl p-5 card-shadow transition-all flex flex-col justify-between text-left h-full"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-slate-500 bg-slate-50 border border-border px-2.5 py-1 rounded-md">
                  ...
                  {activeRole === "student" ? (
                    ...
                    <span className="text-[10px] font-bold text-slate-600 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-full">
                  ...
                  !submission ? (
                    ...
                    <button
                      onClick={() => {
                       ...
                       className="px-3.5 py-2 text-[12px] font-bold text-slate-700 bg-white hover:bg-slate-50 border border-border rounded-lg shadow-sm transition-colors cursor-pointer"
                    >
                      View Submission
                    </button>
                  )
      ...
      <Modal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="New Homework Assignment">
        <form onSubmit={handleAddSubmit} className="space-y-4">
          <div className="flex flex-col gap-1.5 text-left">
            <label className="text-[11px] font-semibold uppercase text-slate-500">Assignment Title</label>
            <input
              required
              type="text"
              value={title}
          ...
          <div className="flex flex-col gap-1.5 text-left">
            <label className="text-[11px] font-semibold uppercase text-slate-500">Task Details & Description</label>
            <textarea
              required
              value={description}
              ...
              className="px-3.5 py-2.5 border border-border rounded-lg bg-white text-[13px] text-slate-900 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm resize-none"
            />
          </div>

          <div className="grid grid-cols-3 gap-4">
            <div className="flex flex-col gap-1.5 text-left">
              <label className="text-[11px] font-semibold uppercase text-slate-500">Class Target</label>
              <select
                value={classId}
            ...
            <div className="flex flex-col gap-1.5 text-left">
              <label className="text-[11px] font-semibold uppercase text-slate-500">Subject</label>
              <input
                required
                type="text"
                value={subject}
            ...
            <div className="flex flex-col gap-1.5 text-left">
              <label className="text-[11px] font-semibold uppercase text-slate-500">Due Date</label>
              <input
                required
                type="date"
                value={dueDate}
                onChange={(e) => setDueDate(e.target.value)}
                 className="px-3.5 py-2.5 border border-border rounded-lg bg-white text-[13px] text-slate-900 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm font-mono font-bold text-slate-600 cursor-pointer"
              />
            </div>
          </div>
      ...
      <Modal isOpen={isSubmitOpen} onClose={() => setIsSubmitOpen(false)} title="Submit Assignment Response">
        <form onSubmit={handleSubmitWork} className="space-y-4">
           <div className="p-4 bg-slate-50 rounded-xl border border-border text-left">
            <h4 className="font-semibold text-slate-900 text-[14px]">{activeHw?.title}</h4>
            <p className="mt-1.5 leading-relaxed text-[13px] text-slate-600">{activeHw?.description}</p>
          </div>

          <div className="flex flex-col gap-1.5 text-left">
            <label className="text-[11px] font-semibold uppercase text-slate-500">Your Response / Essay</label>
            <textarea
              required
              ...
              placeholder="Write your assignment text here..."
               className="px-3.5 py-3 border border-border rounded-lg bg-white text-[13px] text-slate-900 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm resize-none leading-relaxed"
            />
          </div>
      ...
      >
        <div className="space-y-5">
           <div className="text-left space-y-1 bg-slate-50 p-4 rounded-xl border border-border">
            <h3 className="font-semibold text-[14px] text-slate-900">Assignment Overview</h3>
            <p className="text-[13px] text-slate-600 mt-1">{activeHw?.description}</p>
          </div>

          <div className="border border-border rounded-xl overflow-hidden text-left card-shadow">
            <table className="w-full text-[13px] border-collapse">
              <thead>
                 <tr className="bg-slate-50/50 border-b border-border text-[11px] uppercase font-bold text-slate-500">
               ...
               <tbody className="divide-y divide-border bg-white">
                {activeHw?.submissions.length === 0 ? (
                  ...
                  activeHw?.submissions
                    ...
                    .map((sub) => (
                       <tr key={sub.studentId} className="align-top hover:bg-slate-50/50 transition-colors">
                        <td className="px-5 py-4 font-bold text-slate-900">
                          {getStudentName(sub.studentId)}
                        </td>
                         <td className="px-5 py-4 space-y-3 max-w-sm">
                           <p className="text-slate-700 bg-slate-50 p-3 rounded-lg border border-border leading-relaxed text-[12px] shadow-sm">
                            {sub.content}
                          </p>
                          {sub.feedback && (
                             <div className="flex gap-2 items-start text-[11px] text-slate-600 bg-indigo-50/50 p-2.5 rounded-lg border border-indigo-100">
      ...
      <Modal isOpen={isGradeOpen} onClose={() => setIsGradeOpen(false)} title="Evaluate Student Assignment">
        <form onSubmit={handleGradeSubmit} className="space-y-4">
           <div className="p-4 bg-slate-50 rounded-xl border border-border text-left space-y-1.5">
             <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Student Submission</p>
            <p className="font-bold text-[14px] text-slate-900">
              {getStudentName(selectedStudentId)}
            </p>
             <p className="text-slate-600 leading-relaxed text-[13px] pt-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="flex flex-col gap-1.5 text-left col-span-1">
              <label className="text-[11px] font-semibold uppercase text-slate-500">Award Grade</label>
              <select
                value={grade}
                onChange={(e) => setGrade(e.target.value)}
                 className="px-3.5 py-2.5 border border-border rounded-lg bg-white text-[13px] font-bold text-slate-900 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm cursor-pointer"
              >
            ...
            <div className="flex flex-col gap-1.5 text-left col-span-2">
               <label className="text-[11px] font-semibold uppercase text-slate-500">Evaluation Comments</label>
              <input
                required
                type="text"
                value={feedback}
