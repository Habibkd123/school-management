...
const mockSections: Section[] = [
  { id: "SE167645", name: "A", status: "Active" },
  { id: "SE167644", name: "B", status: "Active" },
  { id: "SE167643", name: "C", status: "Active" },
  { id: "SE167642", name: "D", status: "Active" },
  { id: "SE167641", name: "E", status: "Active" },
  { id: "SE167640", name: "F", status: "Active" },
  { id: "SE167639", name: "G", status: "Active" },
  { id: "SE167638", name: "H", status: "Active" },
  { id: "SE167637", name: "I", status: "Active" },
  { id: "SE167636", name: "J", status: "Active" },
];
...
export default function SectionsPage() {
  ...
  const openEditModal = (section: Section) => {
    ...
    setFormStatus(section.status === "Active");
    setIsEditOpen(true);
    ...
    if (selectedSection) {
      setSections(sections.map(s => 
        s.id === selectedSection.id 
          ? { ...s, name: formName, status: formStatus ? "Active" : "Inactive" }
          : s
      ));
    }
    setIsEditOpen(false);
    ...
    <div className="space-y-6 bg-[#F8FAFC] dark:bg-[#0F172A] min-h-screen -m-6 p-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-border rounded-xl shadow-sm overflow-hidden text-left">
        ...
        <div className="p-5 border-b border-border flex flex-col sm:flex-row items-center justify-between gap-4">
          <h2 className="text-[16px] font-bold text-slate-800 dark:text-slate-100">Class Section</h2>
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody className="divide-y divide-border">
              {filteredSections.map((section) => (
                <tr key={section.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4">
                    <input type="checkbox" className="rounded border-slate-300 text-[#F59E0B] focus:ring-[#F59E0B] cursor-pointer" />
                  </td>
                  <td className="px-6 py-4 font-semibold text-[#F59E0B]">{section.id}</td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{section.name}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-[11px] font-bold ${
                      ...
                      <span className={`w-1.5 h-1.5 rounded-full ${section.status === "Active" ? "bg-[#1DD04A]" : "bg-[#E02424]"}`} />
                      {section.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center relative" onClick={(e) => e.stopPropagation()}>
                    <button 
                      onClick={() => setActionMenuId(actionMenuId === section.id ? null : section.id)}
                      className={`p-1.5 rounded-lg transition-colors cursor-pointer ${actionMenuId === section.id ? "bg-[#F59E0B] text-white" : "hover:bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500"}`}
                    >
                    ...
                    {actionMenuId === section.id && (
                      <>
                        ...
                        <div className="absolute right-10 top-10 w-36 bg-white dark:bg-slate-900 border border-border rounded-xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] z-50 overflow-hidden py-2 text-left">
                          <button onClick={() => openEditModal(section)} className="w-full px-4 py-2 text-[13px] text-[#0F172A] dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800/50 flex items-center gap-2 font-medium transition-colors cursor-pointer">
                            <Edit className="w-4 h-4 text-[#0F172A] dark:text-slate-100" /> Edit
                          </button>
                          <button onClick={() => openDeleteModal(section)} className="w-full px-4 py-2 text-[13px] text-[#0F172A] dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800/50 flex items-center gap-2 font-medium transition-colors cursor-pointer">
      ...
      {/* Add / Edit Modals */}
      <Modal isOpen={isAddOpen || isEditOpen} onClose={() => { setIsAddOpen(false); setIsEditOpen(false); }} title={isAddOpen ? "Add Section" : "Edit Section"}>
        <form onSubmit={isAddOpen ? handleAddSubmit : handleEditSubmit} className="p-6 space-y-5 text-left">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            >
              {isAddOpen ? "Add Section" : "Save Changes"}
            </button>
          </div>
        </form>
      </Modal>
