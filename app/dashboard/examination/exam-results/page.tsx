              ...
              {filteredData.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <Link href={`/dashboard/students/${item.id}`} className="flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 p-1.5 -ml-1.5 rounded-lg transition-colors cursor-pointer group w-fit">
                        <Image 
                          src={item.image} 
                          alt={item.name} 
                          width={32} 
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[12px] text-slate-500 dark:text-slate-400 font-medium">Roll No : {item.rollNo}</span>
                        </div>
                      </Link>
                    </td>
                    <td className={`px-6 py-4 font-medium ${item.marks.english < PASS_MARK ? 'text-rose-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.marks.english}</td>
                    <td className={`px-6 py-4 font-medium ${item.marks.spanish < PASS_MARK ? 'text-rose-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.marks.spanish}</td>
                    <td className={`px-6 py-4 font-medium ${item.marks.physics < PASS_MARK ? 'text-rose-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.marks.physics}</td>
                    <td className={`px-6 py-4 font-medium ${item.marks.chemistry < PASS_MARK ? 'text-rose-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.marks.chemistry}</td>
                    <td className={`px-6 py-4 font-medium ${item.marks.maths < PASS_MARK ? 'text-rose-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.marks.maths}</td>
                    <td className={`px-6 py-4 font-medium ${item.marks.computer < PASS_MARK ? 'text-rose-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.marks.computer}</td>
                    <td className={`px-6 py-4 font-medium ${item.marks.envScience < PASS_MARK ? 'text-rose-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.marks.envScience}</td>
