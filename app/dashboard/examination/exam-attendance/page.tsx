  ...
  return (
    <div className="space-y-6 bg-[#F8FAFC] min-h-screen -m-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Exam Attendance</h1>
          <div className="flex items-center gap-2 text-[13px] text-slate-500 mt-1">
            ...
            <span>/</span>
            <span className="text-slate-900 font-medium">Exam Attendance</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="px-6 py-2 bg-[#5D6BEE] hover:bg-[#4b58ce] text-white text-[13px] font-semibold rounded-lg flex items-center gap-2 transition-colors shadow-sm cursor-pointer"
          >
            Save Attendance
          </button>
        </div>
      ...
      <div className="bg-white border border-border rounded-xl shadow-sm overflow-hidden text-left">
        ...
        <div className="p-5 border-b border-border flex flex-col sm:flex-row items-center justify-between gap-4">
          <h2 className="text-[16px] font-bold text-slate-800">Exam Attendance List</h2>
          ...
          <table className="w-full text-[13px]">
            <thead className="bg-[#F8FAFC] border-y border-border">
              <tr>
                ...
                <th className="px-6 py-4 text-left font-bold text-slate-700">Roll No</th>
                <th className="px-6 py-4 text-left font-bold text-slate-700 w-[280px]">Attendance</th>
                <th className="px-6 py-4 text-left font-bold text-slate-700">Note</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {filteredData.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <Link href={`/dashboard/students/${item.id}`} className="flex items-center gap-3 hover:bg-slate-50 p-1 -ml-1 rounded-lg transition-colors cursor-pointer group w-fit">
                      <Image 
                        src={item.image} 
                        alt={item.name} 
                        width={32} 
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2 bg-white border border-border p-1 rounded-lg w-fit shadow-sm">
                      <label className={`cursor-pointer px-4 py-1.5 rounded-md text-[13px] font-semibold transition-all ${item.status === 'Present' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}>
                      ...
                      <label className={`cursor-pointer px-4 py-1.5 rounded-md text-[13px] font-semibold transition-all ${item.status === 'Absent' ? 'bg-rose-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}>
                      ...
                      <label className={`cursor-pointer px-4 py-1.5 rounded-md text-[13px] font-semibold transition-all ${item.status === 'Late' ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}>
                  ...
                  <td className="px-6 py-4">
                    <input 
                      ...
                      className="w-full min-w-[150px] px-3 py-2 text-[13px] border border-transparent hover:border-border focus:border-[#5D6BEE] bg-transparent focus:bg-white rounded-lg outline-none transition-all"
                    />
                  </td>
                </tr>
              ))}
