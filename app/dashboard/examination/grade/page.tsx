...
const mockGradeData: Grade[] = [
  { id: "G180482", grade: "O", percentage: "90% - 100%", gradePoints: "10", status: "Active" },
  { id: "G180481", grade: "A+", percentage: "80% - 90%", gradePoints: "8", status: "Active" },
  { id: "G180480", grade: "A", percentage: "70% - 80%", gradePoints: "6", status: "Active" },
  { id: "G180479", grade: "B+", percentage: "60% - 70%", gradePoints: "4", status: "Active" },
  { id: "G180478", grade: "B", percentage: "50% - 60%", gradePoints: "3", status: "Active" },
  { id: "G180477", grade: "C+", percentage: "40% - 50%", gradePoints: "2", status: "Active" },
  { id: "G180476", grade: "C", percentage: "35% - 40%", gradePoints: "1", status: "Active" },
  { id: "G180475", grade: "F", percentage: "Below 35%", gradePoints: "0", status: "Active" },
];

export default function GradeListPage() {
  const [gradeData, setGradeData] = useState<Grade[]>(mockGradeData);
  ...
  const openEditModal = (item: Grade) => {
    ...
    // Parse percentage to mock from/upto
    let from = "", upto = "";
    if (item.percentage.includes("-")) {
      [from, upto] = item.percentage.split("-").map(s => s.replace("%", "").trim());
    } else if (item.percentage.includes("Below")) {
      upto = item.percentage.replace("Below", "").replace("%", "").trim();
      from = "0";
    }

    setFormMarksFrom(from);
    setFormMarksUpto(upto);
    setFormGradePoints(item.gradePoints);
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const newPercentage = formMarksFrom === "0" ? `Below ${formMarksUpto}%` : `${formMarksFrom}% - ${formMarksUpto}%`;
    const newItem: Grade = {
      id: `G1${Math.floor(10000 + Math.random() * 90000)}`,
      grade: formGrade,
      percentage: newPercentage,
      gradePoints: formGradePoints,
      status: formStatus as "Active" | "Inactive",
    };
    setGradeData([newItem, ...gradeData]);
    setIsAddOpen(false);
  };
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    if (selectedGrade) {
      const newPercentage = formMarksFrom === "0" ? `Below ${formMarksUpto}%` : `${formMarksFrom}% - ${formMarksUpto}%`;
      setGradeData(gradeData.map(s => 
        s.id === selectedGrade.id 
          ? { ...s, grade: formGrade, percentage: newPercentage, gradePoints: formGradePoints, status: formStatus as "Active" | "Inactive" }
          : s
      ));
    }
    setIsEditOpen(false);
  ...
  const handleDeleteConfirm = () => {
    if (selectedGrade) {
      setGradeData(gradeData.filter(s => s.id !== selectedGrade.id));
    }
    setIsDeleteOpen(false);
  };

  const filteredData = gradeData.filter(s => 
  ...
  return (
    <div className="space-y-6 bg-[#F8FAFC] min-h-screen -m-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Grade</h1>
          <div className="flex items-center gap-2 text-[13px] text-slate-500 mt-1">
            ...
            <span>/</span>
            <span className="text-slate-900 font-medium">Grade</span>
          </div>
        </div>
      ...
      <div className="bg-white border border-border rounded-xl shadow-sm overflow-hidden text-left">
        ...
        <div className="p-5 border-b border-border flex flex-col sm:flex-row items-center justify-between gap-4">
          <h2 className="text-[16px] font-bold text-slate-800">Grade List</h2>
                  ...
                  <div className="absolute right-0 sm:left-0 sm:right-auto top-full mt-2 w-64 bg-white border border-border rounded-lg shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] z-50 text-left">
                    ...
                    <div className="p-4 space-y-4">
                      <div className="space-y-1.5">
                        <label className="text-[13px] font-bold text-slate-800">Grade</label>
                        <div className="relative">
          ...
          <table className="w-full text-[13px]">
            <thead className="bg-[#F8FAFC] border-y border-border">
              <tr>
                ...
                <th className="px-6 py-4 text-left font-bold text-slate-700">Percentage</th>
                <th className="px-6 py-4 text-left font-bold text-slate-700">Grade Points</th>
            ...
            <tbody className="divide-y divide-border">
              {filteredData.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-600">{item.gradePoints}</td>
                  <td className="px-6 py-4">
      ...
      <Modal isOpen={isAddOpen || isEditOpen} onClose={() => { setIsAddOpen(false); setIsEditOpen(false); }} title={isAddOpen ? "Add Grade" : "Edit Grade"}>
        <form onSubmit={isAddOpen ? handleAddSubmit : handleEditSubmit} className="p-6 space-y-5 text-left max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-1.5">
            <label className="text-[13px] font-bold text-slate-800">Marks From(%)</label>
            <div className="relative">
              <select 
                value={formMarksFrom}
          ...
          <div className="space-y-1.5">
            <label className="text-[13px] font-bold text-slate-800">Marks Upto(%)</label>
            <div className="relative">
              <select 
                value={formMarksUpto}
          ...
          <div className="space-y-1.5">
            <label className="text-[13px] font-bold text-slate-800">Grade Points</label>
            <div className="relative">
              <select 
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
            ...
            >
              {isAddOpen ? "Add Grade" : "Save Changes"}
            </button>
          </div>
        </form>
      </Modal>
