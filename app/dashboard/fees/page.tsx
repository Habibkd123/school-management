  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
         <div className="bg-white border border-border rounded-xl p-6 card-shadow text-left flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
               <span className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Total Collected</span>
              <span className="text-2xl font-bold block text-slate-900 mt-0.5">${collected.toLocaleString()}</span>
            </div>
          </div>
         ...
         <div className="bg-white border border-border rounded-xl p-6 card-shadow text-left flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
               <span className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Pending Dues</span>
               <span className="text-2xl font-bold block text-slate-900 mt-0.5">${pending.toLocaleString()}</span>
            </div>
          </div>
         ...
         <div className="bg-white border border-border rounded-xl p-6 card-shadow text-left flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
               <span className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Overdue Balances</span>
        ...
        <div className="relative text-left">
          ...
          <input
            ...
            onChange={(e) => setSearch(e.target.value)}
             className="w-full pl-10 pr-4 py-2.5 text-[13px] text-slate-900 bg-white border border-border rounded-xl outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 shadow-sm transition-all"
          />
        </div>
         ...
         <div className="bg-white border border-border rounded-xl card-shadow overflow-hidden text-left">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                 <tr className="border-b border-border bg-slate-50/50 text-[11px] uppercase font-bold text-slate-500 tracking-wider">
               ...
               <tbody className="divide-y divide-border text-[13px]">
                {visibleInvoices.length === 0 ? (
                  <tr>
                    <td colSpan={activeRole === "student" ? 5 : 6} className="px-6 py-12 text-center text-slate-500">
                     ...
                     <tr key={invoice.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                           <span className="font-bold text-slate-900">{invoice.title}</span>
                           <span className="text-[12px] text-slate-500 mt-0.5">Inv: {invoice.id}</span>
                        </div>
                      </td>
                      {activeRole !== "student" && (
                       ...
                       <td className="px-6 py-4 font-mono text-slate-500">{invoice.dueDate}</td>
                       <td className="px-6 py-4 font-bold font-mono text-slate-800">${invoice.amount.toLocaleString()}</td>
                      <td className="px-6 py-4">
                        <span
                      ...
                      <td className="px-6 py-4 text-right">
                        {invoice.status !== "Paid" ? (
                          ...
                          >
                           ...
                           <span className="text-[12px] font-bold text-slate-400 flex items-center justify-end gap-1.5">
      ...
      <Modal isOpen={isPayOpen} onClose={() => setIsPayOpen(false)} title="Checkout Sandbox Gateway">
        <form onSubmit={handlePaymentSubmit} className="space-y-5">
           <div className="p-5 bg-slate-900 text-white rounded-xl text-left shadow-lg space-y-4">
            <div className="flex justify-between items-start">
              <div>
                 <span className="text-[11px] uppercase font-bold text-slate-400 tracking-wider">Checkout Total</span>
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-1.5 text-left">
               <label className="text-[11px] font-semibold uppercase text-slate-500">Cardholder Name</label>
              <input
                required
                type="text"
                value={cardName}
            ...
            <div className="flex flex-col gap-1.5 text-left">
               <label className="text-[11px] font-semibold uppercase text-slate-500">Credit Card Number</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="4242 4242 4242 4242"
                   className="w-full pl-10 pr-3.5 py-2.5 border border-border rounded-lg bg-white text-[13px] text-slate-900 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm font-mono"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5 text-left">
                 <label className="text-[11px] font-semibold uppercase text-slate-500">Expiration Date</label>
                <input
                  required
                  type="text"
                  value={expiry}
                  ...
                  placeholder="MM/YY"
                   className="px-3.5 py-2.5 border border-border rounded-lg bg-white text-[13px] text-slate-900 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm font-mono"
                />
              </div>

              <div className="flex flex-col gap-1.5 text-left">
                 <label className="text-[11px] font-semibold uppercase text-slate-500">CVC Code</label>
                <input
                  required
                  type="text"
                  value={cvc}
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            ...
            >
             ...
             <p className="text-[11px] text-slate-500 text-center flex items-center justify-center gap-1 mt-1">
      ...
      <Modal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Generate billing Invoice">
        <form onSubmit={handleCreateInvoiceSubmit} className="space-y-4">
          <div className="flex flex-col gap-1.5 text-left">
             <label className="text-[11px] font-semibold uppercase text-slate-500">Invoice statement Title</label>
            <input
              required
              type="text"
              value={invoiceTitle}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="flex flex-col gap-1.5 text-left col-span-1">
               <label className="text-[11px] font-semibold uppercase text-slate-500">Billing Amount ($)</label>
              <input
                required
                type="number"
                value={amount}
            ...
            <div className="flex flex-col gap-1.5 text-left col-span-1">
              ...
              <input
                ...
                onChange={(e) => setDueDate(e.target.value)}
                 className="px-3.5 py-2.5 border border-border rounded-lg bg-white text-[13px] text-slate-900 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm font-mono text-slate-600 font-bold"
              />
            </div>
          </div>
