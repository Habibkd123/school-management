      ...
      <div className="flex-1 flex flex-col relative bg-white dark:bg-slate-900">
        ...
        <div className="flex-1 flex flex-col justify-center items-center px-6 sm:px-12 lg:px-16 xl:px-24 pb-20">
          <div className="w-full max-w-[420px] bg-white dark:bg-slate-900 border border-border/60 dark:border-slate-800 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8">
            ...
            <div className="mb-6">
              <h1 className="text-[24px] font-bold text-[#202c4b] dark:text-slate-100 mb-2">Welcome</h1>
              <p className="text-[13px] text-slate-500 dark:text-slate-400">Please enter your details to sign in</p>
            </div>
            ...
            <div className="flex gap-2 mb-6">
              <button 
                type="button"
                onClick={() => { setEmail("admin@preskool.com"); setPassword("admin123"); }} 
                className="flex-1 py-2 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[12px] font-bold hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-colors"
              >
                Admin
              </button>
              <button 
                type="button"
                onClick={() => { setEmail("teacher@preskool.com"); setPassword("teacher123"); }} 
                className="flex-1 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[12px] font-bold hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-colors"
              >
                Teacher
              </button>
              <button 
                type="button"
                onClick={() => { setEmail("student@preskool.com"); setPassword("student123"); }} 
                className="flex-1 py-2 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[12px] font-bold hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors"
              >
                Student
              </button>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex items-center justify-between pt-1 pb-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-[12px] text-slate-500 dark:text-slate-400 font-medium">Remember Me</span>
                </label>
                <a href="/forget-password" className="text-[12px] font-medium text-rose-500 hover:text-rose-600 transition-colors">
                  Forgot Password?
                </a>
              </div>
            ...
            <p className="w-full text-center text-[13px] text-slate-600 dark:text-slate-300 font-medium mt-6">
              ...
              <Link href="/register" className="text-[#5D6BEE] hover:text-[#4b58ce] transition-colors">
                Create Account
              </Link>
            </p>
          </div>
